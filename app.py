...
try:
    ...
    go = None  # requirements.txt: plotly>=5.18,<6 필요
...
if mode == "공급량 예측":
    ...
    def _render_with_year_sums(title, table, temp_col_name):
        ...
        # 연도별 총계(월/월평균기온은 공란으로)
        prods_in_table = [c for c in table.columns if c not in ["연","월",temp_col_name]]
        sums = table.groupby("연")[prods_in_table].sum(numeric_only=True).reset_index()
        sums.insert(1, "월", "")                # 공란
        sums.insert(2, "월평균기온", "")        # 공란
        st.markdown("#### 연도별 총계")
        render_centered_table(sums, int_cols=prods_in_table, index=False)
        return sums
    ...
    st.download_button(
        ...
        data=to_dl_csv.to_csv(index=False).encode("utf-8-sig"),
        file_name="citygas_supply_forecast_all_scenarios.csv",
        mime="text/csv"
    )

    # 엑셀(xlsx) — 월별표/연도별총계 각각 시트
    xbuf = BytesIO()
    with pd.ExcelWriter(xbuf, engine="openpyxl") as xw:
        tbl_n.to_excel(xw, index=False, sheet_name="Normal")
        tbl_b.to_excel(xw, index=False, sheet_name="Best")
        tbl_c.to_excel(xw, index=False, sheet_name="Conservative")
        tbl_trd.to_excel(xw, index=False, sheet_name="Trend")
        sum_n.to_excel(xw, index=False, sheet_name="Normal_Yearly")
        sum_b.to_excel(xw, index=False, sheet_name="Best_Yearly")
        sum_c.to_excel(xw, index=False, sheet_name="Cons_Yearly")
        sum_t.to_excel(xw, index=False, sheet_name="Trend_Yearly")
    st.download_button(
        "⬇️ 예측 결과 Excel 다운로드 (월별표 + 연도별 총계)",
        data=xbuf.getvalue(),
        file_name="citygas_supply_forecast_all_scenarios.xlsx",
    ...
    cyr1, cyr2, cyr3 = st.columns([1,1,1])
    with cyr1:
    ...
    for prod in prods:
        ...
        P_norm = fut_base[["연","월"]].copy()
        P_norm["pred"] = np.clip(np.rint(y_future_norm).astype(np.int64), a_min=0, a_max=None)
        # Trend 예측
        ...
        P_trend = fut_base[["연","월"]].copy()
        P_trend["pred"] = np.clip(np.rint(y_future_trd).astype(np.int64), a_min=0, a_max=None)
    ...
    st.caption("ℹ️ **95% 신뢰구간**: 잔차 표준편차 기반 근사 예측구간으로, 새로운 관측이 약 95% 확률로 이 음영 범위에 들어옵니다.")

# =============== B) 판매량 예측(냉방용) =====================================
else:
    # (판매 파트 기존 로직 유지 — 파일/전처리/예측 로직을 아래에 그대로 이어 붙이면 됩니다)
