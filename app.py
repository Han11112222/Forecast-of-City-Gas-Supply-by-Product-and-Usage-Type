# app.py — 도시가스 공급·판매 분석 (Poly-3 + Poly-4 비교) + 아이콘 강화판
...
# 분석 유형
with st.sidebar:
    title_with_icon("🧭", "분석 유형", "h3", small=True)
    mode = st.radio("🔀 선택", ["공급량 분석", "판매량 분석(냉방용)"], index=0, label_visibility="visible")
    ...
    with st.sidebar:
        ...
        if src == "Repo 내 파일 사용":
            ...
            if any(p.exists() for p in fc_candidates):
                ...
                up_fc = st.file_uploader("🌡️ 예상기온 업로드(xlsx) — (날짜, 평균기온)", type=["xlsx"], key="up_fc_repo")
            ...
            up_fc = st.file_uploader("🌡️ 예상기온 엑셀 업로드(xlsx) — (날짜, 평균기온)", type=["xlsx"])
            if up_fc is not None:
                forecast_df = read_temperature_forecast(up_fc)
