...
def normalize_cols(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in df.columns:
        if df[c].dtype == "object":
            df[c] = pd.to_numeric(df[c].astype(str).str.replace(",","",regex=False).str.replace(" ","",regex=False),
                                  errors="ignore")
    return df
...
def read_temperature_raw(file):
    ...
    for i in range(len(head)):
        ...
        if any(v in ["날짜","일자","date","Date"] for v in row) and any(("평균기온" in v) or ("기온" in v) or
           (isinstance(v,str) and v.lower() in ["temp","temperature"]) for v in row):
            header_row = i; break
...
def poly_eq_text(model):
    c = model.coef_
    c1 = c[0] if len(c)>0 else 0.0; c2 = c[1] if len(c)>1 else 0.0; c3 = c[2] if len(c)>2 else 0.0
    d = model.intercept_
    return f"y = {c3:+.5e}x³ {c2:+.5e}x² {c1:+.5e}x {d:+.5e}"

def poly_eq_text4(model):
    c = model.coef_
    c1 = c[0] if len(c)>0 else 0.0; c2 = c[1] if len(c)>1 else 0.0; c3 = c[2] if len(c)>2 else 0.0; c4 = c[3] if len(c)>3 else 0.0
    d = model.intercept_
...
if mode == "공급량 분석":
    ...
    with st.sidebar:
        ...
        if src == "Repo 내 파일 사용":
            ...
            default_idx = next((i for i,p in enumerate(repo_files) if "상품별공급량" in Path(p).stem), 0) if repo_files else 0
            if repo_files:
    ...
    if forecast_df is None or forecast_df.empty:
        st.info("좌측에서 예상기온 엑셀을 선택/업로드하세요."); st.stop()
    ...
    if run_btn:
        ...
        if miss.any():
            ...
            st.warning("기온예측에 없는 월은 학습데이터의 월평균으로 보강했어.")
    ...
    for prod in prods:
        ...
        ax.text(0.02, 0.96, f"Poly-3: {poly_eq_text(model)}", transform=ax.transAxes, ha="left", va="top", fontsize=9,
...
# =================== B) 판매량 분석(냉방용) ===================
    ...
    # 이하 판매량 분석(그래프/검증/Poly-4 비교)은 이전 버전 그대로 …
    sm = st.session_state["sales_materials"]
    # (생략 없이 전체 구현되어 있음 — 위 코드 그대로 사용)
