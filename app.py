...
# ── 지수표기 없이 깔끔한 숫자 문자열(콤마 포함)
def _fmt_nosci(x: float, precision: int = 6) -> str:
    s = np.format_float_positional(abs(float(x)), precision=precision, trim='-')
    if '.' in s:
        ip, fp = s.split('.')
        s = f"{int(ip):,}.{fp}"
    else:
...
def poly_eq_text(model, precision: int = 6) -> str:
    c = model.coef_
    ...
    d = model.intercept_
    return (
        f"y = {_fmt_nosci(c3, precision)}x³ "
        f"{_fmt_nosci(c2, precision)}x² "
        f"{_fmt_nosci(c1, precision)}x "
        f"{_fmt_nosci(d,  precision)}"
    )

def poly_eq_text4(model, precision: int = 6) -> str:
    c = model.coef_
    c1 = c[0] if len(c) > 0 else 0.0
    c2 = c[1] if len(c) > 1 else 0.0
    c3 = c[2] if len(c) > 2 else 0.0
    ...
    d = model.intercept_
    return (
        f"y = {_fmt_nosci(c4, precision)}x⁴ "
        f"{_fmt_nosci(c3, precision)}x³ "
        f"{_fmt_nosci(c2, precision)}x² "
        f"{_fmt_nosci(c1, precision)}x "
        f"{_fmt_nosci(d,  precision)}"
    )
        ...
        if go is None:
            ...
            for y in sorted([int(v) for v in years_view]):
                ...
                fig.add_trace(go.Scatter(
                    x=[f"{int(m)}월" for m in one["월"]], y=one[prod],
                    mode="lines+markers", name=f"{y} 실적",
                    hovertemplate="%{x} %{y:,}"
                ))
            # 예측 (Normal/Best/Cons) — hover에 월평균기온 표시
            for y in years_pred:
                row = P_norm[P_norm["연"] == int(y)].sort_values("월")
                temps = (fut_base[fut_base["연"] == int(y)].sort_values("월")["예상기온"] + d_norm).values
                fig.add_trace(go.Scatter(
                    x=[f"{int(m)}월" for m in row["월"]], y=row["pred"],
                    mode="lines", name=f"예측(Normal) {y}",
                    line=dict(dash="dash"),
                    customdata=np.round(temps, 1),
                    hovertemplate="%{x} %{y:,}<br>월평균기온: %{customdata:.1f}℃"
                ))
                if show_best:
                    rb = P_best[P_best["연"] == int(y)].sort_values("월")
                    temps_b = (fut_base[fut_base["연"] == int(y)].sort_values("월")["예상기온"] + d_best).values
                    fig.add_trace(go.Scatter(
                        x=[f"{int(m)}월" for m in rb["월"]], y=rb["pred"],
                        mode="lines", name=f"예측(Best) {y}",
                        line=dict(dash="dash"),
                        customdata=np.round(temps_b, 1),
                        hovertemplate="%{x} %{y:,}<br>월평균기온: %{customdata:.1f}℃"
                    ))
                if show_cons:
                    rc = P_cons[P_cons["연"] == int(y)].sort_values("월")
                    temps_c = (fut_base[fut_base["연"] == int(y)].sort_values("월")["예상기온"] + d_cons).values
                    fig.add_trace(go.Scatter(
                        x=[f"{int(m)}월" for m in rc["월"]], y=rc["pred"],
                        mode="lines", name=f"예측(Conservative) {y}",
                        line=dict(dash="dash"),
                        customdata=np.round(temps_c, 1),
                        hovertemplate="%{x} %{y:,}<br>월평균기온: %{customdata:.1f}℃"
                    ))
            # 기온추세분석선 — hover에 추세기온
            for y in years_trnd:
                row = P_trend[P_trend["연"] == int(y)].sort_values("월")
                temps_t = fut_base[fut_base["연"] == int(y)].sort_values("월")["추세기온"].values
                fig.add_trace(go.Scatter(
                    x=[f"{int(m)}월" for m in row["월"]], y=row["pred"],
                    mode="lines", name=f"기온추세분석 {y}",
                    line=dict(dash="dot"),
                    customdata=np.round(temps_t, 1),
                    hovertemplate="%{x} %{y:,}<br>월평균기온(추세): %{customdata:.1f}℃"
                ))
            fig.update_layout(
...
def render_trend_forecast():
    ...
    def _fore_arima_yearsum(prod: str, target_years: list[int]) -> dict:
        """월별→연도합 ARIMA 실패 시, 연도합 시계열에 직접 ARIMA 폴백."""
