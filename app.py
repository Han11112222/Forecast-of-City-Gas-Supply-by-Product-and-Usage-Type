...
# 표시 순서(표에서도 이 순서로 정렬) — 중앙난방용을 개별난방용 바로 뒤에 배치
KNOWN_PRODUCT_ORDER = [
...
if mode == "공급량 분석":
    with st.sidebar:
        ...
        if src == "Repo 내 파일 사용":
            ...
            if repo_files:
                default_idx = next((i for i, p in enumerate(repo_files) if "상품별공급량" in p), 0)
    ...
    if run_btn:
        ...
        if scen == "학습기간 월별 평균":
            ...
            base_temp = (
                base[base["연"] == last_train_year][["월", temp_col]]
                .groupby("월")[temp_col]
                .mean()
                .rename("temp")
                .reset_index()
        ...
        for col in prods:
            ...
            tmp = pred.copy()
            tmp["상품"] = col
            tmp = tmp.rename(columns={"pred": "예측공급량"})
            pred_rows.append(tmp)

        if pred_rows:
            pivot = (
                pd.concat(pred_rows, ignore_index=True)
                .pivot_table(index=["연", "월"], columns="상품", values="예측공급량")
                .reset_index()
            )
            # 컬럼 순서: KNOWN_PRODUCT_ORDER(선택된 상품만) → 기타
            ordered_cols = [c for c in KNOWN_PRODUCT_ORDER if c in prods and c in pivot.columns]
            others = [c for c in pivot.columns if c not in ordered_cols + ["연", "월"]]
            pivot = pivot[["연", "월"] + ordered_cols + others]

            # 정수 형식
            for c in pivot.columns:
                if c not in ["연", "월"]:
                    pivot[c] = pd.to_numeric(pivot[c], errors="coerce").round().astype("Int64")

            # 종계 행
            total = {c: (pivot[c].sum() if c not in ["연", "월"] else None) for c in pivot.columns}
            total["연"], total["월"] = "종계", ""
            pivot_tot = pd.concat([pivot, pd.DataFrame([total])], ignore_index=True)

            result["pred_table"] = pivot_tot
    ...
    if res["pred_table"] is not None:
        ...
        render_centered_table(res["pred_table"], int_cols=int_cols, index=False)
        st.download_button(
            "예측 결과 CSV 다운로드",
...
# ======================================================================
    ...
    if run_btn:
        ...
        def period_avg(label_m: pd.Timestamp) -> float:
            ...
            s = (m - pd.offsets.MonthBegin(1)) + pd.DateOffset(days=15)  # 전월 16
            e = m + pd.DateOffset(days=14)                               # 당월 15
        ...
        out["오차율(%)"] = (out["오차"] / out["실제판매량"] * 100.0).astype(float)
    ...
    show["연"] = show["연"].astype(int).astype(str)
    show["월"] = show["월"].astype("Int64")
    forecast_tbl = show[["연", "월", "당월평균기온", "기간평균기온 (m-1, 16일 ~ m15일)", "예측판매량"]].copy()

    # 종계 행(예측)
    sum_pred = int(pd.to_numeric(forecast_tbl["예측판매량"], errors="coerce").sum())
    tot_row_fc = {
        "연": "종계",
        "월": "",
        "당월평균기온": np.nan,
        "기간평균기온 (m-1, 16일 ~ m15일)": np.nan,
        "예측판매량": sum_pred,
    }
    forecast_tbl_tot = pd.concat([forecast_tbl, pd.DataFrame([tot_row_fc])], ignore_index=True)
    ...
    st.download_button(
        "판매량 예측 CSV 다운로드",
        data=forecast_tbl_tot.to_csv(index=False).encode("utf-8-sig"),
    ...
    verify_tbl = res["pred"][["연", "월", "실제판매량", "예측판매량", "오차", "오차율(%)"]].copy()
    verify_tbl["연"] = verify_tbl["연"].astype(int).astype(str)
    verify_tbl["월"] = verify_tbl["월"].astype("Int64")

    # 종계(검증)
    tot_act = int(pd.to_numeric(verify_tbl["실제판매량"], errors="coerce").sum())
    tot_pre = int(pd.to_numeric(verify_tbl["예측판매량"], errors="coerce").sum())
    tot_err = int(tot_pre - tot_act)
    tot_rate = (tot_err / tot_act * 100.0) if tot_act else np.nan
    tot_row_vf = {
        "연": "종계",
        "월": "",
        "실제판매량": tot_act,
    ...
    verify_tbl_tot = pd.concat([verify_tbl, pd.DataFrame([tot_row_vf])], ignore_index=True)
    render_centered_table(
        verify_tbl_tot,
        float1_cols=["오차율(%)"],
    ...
    xtr, ytr = res["train_xy"]
    poly = PolynomialFeatures(degree=3, include_bias=False)
    Xtr = poly.fit_transform(xtr.reshape(-1, 1))
    mdl = LinearRegression().fit(Xtr, ytr)
    r2 = mdl.score(Xtr, ytr)

    xs = np.linspace(min(xtr) - 1, max(xtr) + 1, 200)
    ys = mdl.predict(poly.transform(xs.reshape(-1, 1)))

    fig = plt.figure(figsize=(9, 3.6))
    ax = plt.gca()
    ax.scatter(xtr, ytr, s=18, label="학습 샘플")
    ax.plot(xs, ys, linestyle="--", label=f"Poly-3 회귀선 (R²={r2:.3f})")
    ax.set_xlabel("기간평균기온 (m-1,16~m15)")
    ax.set_ylabel("냉방용 판매량")
    ax.legend(loc="best")
    plt.tight_layout()
    st.pyplot(fig, clear_figure=True)
