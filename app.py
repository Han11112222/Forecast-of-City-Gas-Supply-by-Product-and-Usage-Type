# app.py — 도시가스 공급·판매 예측 (3섹션 + 추천 학습연도)
...
# D) 추천 학습연도       : 상품별 r·R²로 학습연도 추천 → 적용 시 공급량 예측 탭 기본값 세팅
...
.badge { display:inline-block; padding:.25rem .5rem; border-radius:.5rem; background:#eef; margin:.15rem .3rem .15rem 0; font-size:.85rem; }
</style>
...
def render_supply_forecast():
    with st.sidebar:
        ...
        # ▼ 추천 학습연도 적용(세션에 있으면 그걸 기본값으로)
        years_sel = st.multiselect("🗓️ 연도 선택", years_all, default=st.session_state.get("reco_years", years_all))
...
def render_recommend_learn_years():
    ...
    st.caption("상품(용도)별로 **기온↔공급량** 관계를 월단위로 분석해서 R²·상관도(r)를 보여주고, 기준을 넘어서는 연도를 추천합니다.")
    ...
    if df is None or df.empty:
        st.info("👈 좌측에서 실적 파일을 선택/업로드하면 분석을 시작할 수 있습니다.")
        st.stop()
    ...
    temp_col = detect_temp_col(df)
    if temp_col is None:
        st.error("🌡️ 실적 파일에서 '평균기온/기온' 열을 찾지 못했습니다. 데이터에 기온 열을 추가해주세요.")
        st.stop()

    product_cols = guess_product_cols(df)
    title_with_icon("📦", "상품(용도) 선택 — 토글로 여러 개 고르기", "h3", small=True)
    ...
    for prod in selected_products:
        ...
        st.markdown(f"**👉 {prod} 추천 학습연도:** " + ", ".join(map(str, cand)) if cand else "추천 가능 연도가 충분치 않습니다.")
    ...
    if st.button("💾 공급량 예측 탭의 기본 학습연도로 **적용하기**", type="primary", key="apply_reco"):
        if reco_sorted:
            ...
            st.info("추천 목록이 비어 있어 적용할 값이 없습니다.")
...
def main():
    ...
    with st.sidebar:
        ...
        mode = st.radio(
            "🔀 선택",
            ["공급량 예측", "판매량 예측(냉방용)", "공급량 추세분석 예측", "추천 학습연도"],
            index=0,
