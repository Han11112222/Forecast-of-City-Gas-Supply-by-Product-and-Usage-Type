# app.py — 도시가스 공급·판매 분석 (Poly-3 + Poly-4 비교) — 사이드바 완전 고정 레이아웃
...
# ─────────────────────────────────────────────────────────────
# 0) 공통: 좌측 사이드바에 모든 입력 배치
with st.sidebar:
    title_with_icon("🧭", "분석 유형", 2)
    mode = st.radio("선택", ["공급량 분석", "판매량 분석(냉방용)"], index=0)

# =============== A) 공급량 분석 ==========================================
if mode == "공급량 분석":

    # ---- 사이드바: 데이터/연도/상품/예측설정/버튼 전부
    with st.sidebar:
        title_with_icon("📂", "데이터 불러오기", 3)
        ...
        if df is not None and len(df):
            title_with_icon("📚", "학습 데이터 연도 선택", 3)
            ...
            if temp_col is None:
                st.warning("기온 열을 찾지 못했습니다. 열 이름에 '평균기온' 또는 '기온' 필요.")

            title_with_icon("🏷️", "예측할 상품 선택", 3)
            product_cols = guess_product_cols(df) if temp_col else []
            ...
            title_with_icon("🌡️", "시나리오 Δ°C (평균기온 보정)", 3)
            ...
            if run_btn:
                ...
                if temp_col is None or train_df.empty or not prods:
                    st.error("필수 설정이 부족합니다. (기온열/연도/상품 확인)")
                    ...
                    if f_end < f_start:
                        ...
                        st.session_state["supply_materials"] = dict(
                            ...
                            d_norm=d_norm, d_best=d_best, d_cons=d_cons
                        )
                        st.success("공급량 예측(베이스) 준비 완료! 본문에서 결과를 확인하세요.")
        else:
            st.info("엑셀을 선택/업로드하면 나머지 설정이 열립니다.")

    # ---- 본문: 결과만 출력
    if "supply_materials" not in st.session_state:
        st.info("좌측 사이드바에서 입력을 완료하고 **예측 시작**을 눌러 실행하세요.")
        st.stop()
    ...
    d_norm, d_best, d_cons = mats["d_norm"], mats["d_best"], mats["d_cons"]
    ...
    # 표 출력 + 다운로드
    title_with_icon("📄", "예측 결과 표", 2)
    st.markdown("#### Normal")
    tbl_n = _forecast_table_for_delta(d_norm); render_centered_table(
        tbl_n, float1_cols=["월평균기온"],
        int_cols=[c for c in tbl_n.columns if c not in ["연","월","월평균기온"]], index=False)

    st.markdown("#### Best")
    tbl_b = _forecast_table_for_delta(d_best); render_centered_table(
        tbl_b, float1_cols=["월평균기온"],
        int_cols=[c for c in tbl_b.columns if c not in ["연","월","월평균기온"]], index=False)

    st.markdown("#### Conservative")
    tbl_c = _forecast_table_for_delta(d_cons); render_centered_table(
        tbl_c, float1_cols=["월평균기온"],
        int_cols=[c for c in tbl_c.columns if c not in ["연","월","월평균기온"]], index=False)

    def add_scenario(df, name): d = df.copy(); d.insert(0, "시나리오", name); return d
    tbl_all = pd.concat([add_scenario(tbl_n,"Normal"), add_scenario(tbl_b,"Best"), add_scenario(tbl_c,"Conservative")], ignore_index=True)
    st.download_button(
    ...
    # 그래프
    title_with_icon("📈", "그래프 (Normal 기준)", 2)
    ...
    for prod in prods:
        ...
        st.caption("**95% 신뢰구간(근사 예측구간)**: 잔차 표준편차 s를 사용해 `예측값 ± 1.96·s`로 표시. "
                   "정규·등분산 가정 하에 관측의 약 95%가 이 띠에 포함된다.")

# =============== B) 판매량 분석(냉방용) =====================================
else:
    title_with_icon("🧊", "판매량 분석(냉방용) — 전월 16일 ~ 당월 15일 평균기온 기준", 2)
    ...
    with st.sidebar:
        ...
        def _find_repo_sales_and_temp():
            ...
            temp_candidates  = [ data_dir / "기온.xlsx", *[Path(p) for p in glob(str(data_dir / "*기온*.xlsx"))], *[Path(p) for p in glob(str(data_dir / "*temp*.csv"))] ]
    ...
    if "sales_materials" in st.session_state and not run_btn:
        pass  # 기존 결과 유지
    elif not run_btn:
        st.info("좌측 사이드바에서 파일을 선택/업로드하고 **예측 시작**을 눌러 실행하세요.")
        st.stop()
    ...
    st.success("냉방용 판매량 예측(베이스) 준비 완료! 본문에서 결과를 확인하세요.")
    ...
    d_norm, d_best, d_cons = sm["d_norm"], sm["d_best"], sm["d_cons"]

    title_with_icon("🧮", "다항식 보기 선택", 2)
    ...
    if show_poly3:
        ...
        title_with_icon("📄", "예측 결과 표 — Poly-3", 2)
        st.markdown("#### Normal")
        ...
        title_with_icon("📈", "그래프 (Normal 기준) — Poly-3", 2)
        ...
        title_with_icon("📌", f"기온-냉방용 실적 상관관계 (Train, R²={r2_fit:.3f}) — Poly-3", 2)
        ...
        gb["x"] = [b.mid for b in gb["bin"]]
        ax3.scatter(gb["x"], gb["y"], label="온도별 중앙값", s=65, color="#ff7f0e")
    ...
    if show_poly4:
        ...
        title_with_icon("📄", "예측 결과 표 — Poly-4", 2)
        sale4_n = forecast_sales_table_poly4(d_norm); render_centered_table(sale4_n, float1_cols=["월평균기온(적용)","기간평균기온(적용)"], int_cols=["예측판매량"], index=False)
        sale4_b = forecast_sales_table_poly4(d_best); render_centered_table(sale4_b, float1_cols=["월평균기온(적용)","기간평균기온(적용)"], int_cols=["예측판매량"], index=False)
        sale4_c = forecast_sales_table_poly4(d_cons); render_centered_table(sale4_c, float1_cols=["월평균기온(적용)","기간평균기온(적용)"], int_cols=["예측판매량"], index=False)
        ...
        title_with_icon("📈", "그래프 (Normal 기준) — Poly-4", 2)
        ...
        base_plot4["기간평균기온(적용)"] = base_plot4["기간평균기온"] + d_norm
        ...
        title_with_icon("📌", f"기온-냉방용 실적 상관관계 (Train, R²={r2_fit4:.3f}) — Poly-4", 2)
        ...
        ax34.scatter(gb4["x"], gb4["y"], s=65, label="온도별 중앙값", color="#ff7f0e")
