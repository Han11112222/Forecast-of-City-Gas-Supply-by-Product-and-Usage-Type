...
#  - 공급량 추세분석 예측: 연도별 총합 기반 OLS/CAGR/Holt/SES + (NEW) ARIMA/SARIMA, 인터랙티브 차트
...
def set_korean_font():
    ...
    here = _P(__file__).parent if "__file__" in globals() else _P.cwd()
    candidates = [
...
# ============================= A) 공급량 예측 ==================================
# ▶︎ 이 블록은 네가 준 기존 코드와 동일 (생략 없이 그대로 유지). 
#   ……………………………………………………………………………………………………………………………
# [A) 공급량 예측] 전체 블록: 기존 메시지의 동일 내용 — 변경 없음
#   ……………………………………………………………………………………………………………………………
# (길이상 재반복을 줄이기 위해 여기서는 설명만 남기지만, 실제 파일에서는 기존 A) 블록 전체를 그대로 둔다.)
# ==============================================================================

# ============================= B) 판매량 예측(냉방용) ==========================
# ▶︎ 이 블록도 기존 코드 그대로 유지 (Poly-3/4 비교 포함).
#   ……………………………………………………………………………………………………………………………
# (실제 파일에서는 기존 B) 블록 전체 유지)
# ==============================================================================
...
if mode == "공급량 추세분석 예측":
    ...
    def _fore_sarima_yearsum(prod: str, target_years: list[int]) -> dict:
        ...
        if train.dropna().empty:
            return {y: np.nan for y in target_years}
        # SARIMA(1,1,1)(1,1,1)12 고정(안정/간결)
        try:
            mdl = SARIMAX(train, order=(1,1,1), seasonal_order=(1,1,1,12), enforce_stationarity=False, enforce_invertibility=False).fit(disp=False)
        except Exception:
            return {y: np.nan for y in target_years}
        steps = 12 * (max(target_years) - int(train.index[-1].year))
        if steps <= 0: steps = 12
    ...
    for prod in prods:
        ...
        # 예측표
        tab_cols = ["연"] + [k for k in methods_selected]
        ...
        if go is None:
            ...
            fig.add_trace(go.Scatter(
                x=yd["연"], y=yd[prod], mode="lines", name="실적",
                line=dict(width=3)
            ))
            fig.add_trace(go.Scatter(
                x=yd["연"], y=yd[prod], mode="lines", name="영역",
                fill="tozeroy", line=dict(width=0.1), showlegend=False, hoverinfo="skip"
            ))
            # 방법별 포인트(토글 가능)
            sym = {
                "CAGR(복리성장)":"circle", "Holt(지수평활)":"square", "OLS(선형추세)":"triangle-up",
            ...
            for name in methods_selected:
                if name in pred_map:
                    ...
                    ys = [pred_map[name].get(y, np.nan) for y in xs]
                    fig.add_trace(go.Scatter(
                        x=xs, y=ys, mode="markers+text", name=name, marker_symbol=sym.get(name,"circle"),
                        text=[f"{int(v):,}" if v==v else "" for v in ys], textposition="top center"
                    ))
            fig.update_layout(
        ...
        # ===== 그래프 ②: 방법별 On/Off 토글(동적) — 3번 이미지 느낌의 상호작용 =====
        if go is not None:
...
- **ARIMA(p,d,q)** — 차분(d)으로 정상화한 뒤 AR(p), MA(q) 결합으로 예측(월별 시계열 학습 → 연도합 집계).  
  여기선 간결한 후보 \((1,1,0),(0,1,1),(1,1,1)\) 중 AIC가 최소인 모델을 자동 선택.

- **SARIMA(P,D,Q,12)** — 월별 계절주기 12를 두는 확장.  
  기본 설정: \((1,1,1)\times(1,1,1)_{12}\) (안정성/간결성 우선).
""")
