...
# 반영: 에러 수정(astype[str]→astype(str)), 냉방 전용 컬럼 선택, 적용기온 표기,
#      표에 당월/기간 평균기온(소수 1자리), 그래프(최근5 + 예측1), 한글 폰트 강화
...
def set_korean_font():
    ...
    for p in local_fonts + system_fonts:
        try:
            ...
            continue
    # family와 sans-serif 둘 다 세팅
    families = ["NanumGothic", "Malgun Gothic", "AppleGothic", "Noto Sans CJK KR"]
...
def ym_picker(label: str, default: pd.Timestamp):
    ...
    with c1:
        year = st.selectbox(f"{label} — 연", options=years, index=years.index(int(default.year)))
    with c2:
        month = st.selectbox(f"{label} — 월", options=list(range(1, 13)), index=int(default.month)-1)
    return pd.Timestamp(year=int(year), month=int(month), day=1)

# 최근 5개년 실적 + 예측 12개월 (X축 1~12월)
...
if mode == "공급량 분석":
    ...
    # 표 가독화
    pred_pivot["연"] = pred_pivot["연"].astype(int).astype(str)   # ← 오타 수정: astype(str)
...
# ======================================================================
    ...
    # '냉방' 키워드가 포함된 수치열만 제시 (취사용 제거)
    cand_cool = [c for c in sales_df.columns if ("냉방" in str(c)) and pd.api.types.is_numeric_dtype(sales_df[c])]
    ...
    with s2:
        if not cand_cool:
            st.error("냉방 관련 수치 컬럼을 찾지 못했습니다. 엑셀 열 이름에 '냉방'을 포함시켜 주세요.")
            st.stop()
        # 냉방/냉방용 자동 기본 선택
        default_idx = 0
        for i, c in enumerate(cand_cool):
            if "냉방용" in str(c): default_idx = i; break
        sales_value_col = st.selectbox("냉방용 판매량 컬럼", options=cand_cool, index=default_idx)
    ...
    monthly_temp_by_M = temp_raw.groupby("월")["기온"].mean()  # 동월 평균(보완용)
    ...
    if miss.any():
        sales_join.loc[miss, "기간평균기온"] = sales_join.loc[miss, "판매월"].dt.month.map(monthly_temp_by_M)
    ...
    for m in months:
        ...
        # 당월평균(달력월)
        avg_month = monthly_temp_calendar.loc[
            (monthly_temp_calendar["연"] == m.year) & (monthly_temp_calendar["월"] == m.month), "기온"
        ].mean()
        pred_rows.append({"연": int(m.year), "월": int(m.month), "기간평균기온": avg_period, "당월평균기온": avg_month})
    pred_input = pd.DataFrame(pred_rows)

    # 결측 보완(동월 평균), 그래도 없으면 제외
    for col in ["기간평균기온","당월평균기온"]:
        miss = pred_input[col].isna()
        if miss.any():
            pred_input.loc[miss, col] = pred_input.loc[miss, "월"].map(monthly_temp_by_M)
    ...
    out["당월평균기온"] = pd.to_numeric(out["당월평균기온"], errors="coerce").round(1)
    out["기간평균기온(m-1월16~m15)"] = pd.to_numeric(out["기간평균기온"], errors="coerce").round(1)
    out = out.drop(columns=["기간평균기온"]).rename(columns={"기간평균기온(m-1월16~m15)":"기간평균기온(m-1월16~m15)"})
    st.subheader("판매량 예측(요약)")
    st.dataframe(out[["연","월","당월평균기온","기간평균기온(m-1월16~m15)","예측판매량"]],
                 use_container_width=True, column_config={"연": st.column_config.TextColumn("연")})
