...
try:
    ...
    go = None  # requirements에 plotly>=5.18,<6 추가되어 있어야 함
# =====================================
...
if mode == "공급량 예측":
    ...
    def _render_with_year_sums(title, table, temp_col_name):
        ...
        cols_int = [c for c in sums.columns if c!="연"]
        st.markdown("#### 연도별 총계")
    ...
    def add_scenario(df, name, temp_name):
        out = df.copy(); out.insert(0, "시나리오", name); out.rename(columns={temp_name:"월평균기온"}, inplace=True); return out
    to_dl = pd.concat([
