...
def title_with_icon(icon: str, text: str, level: str = "h1"):
    st.markdown(f"<{level} class='icon-title'><span class='emoji'>{icon}</span><span>{text}</span></{level}>", unsafe_allow_html=True)

# 기존 st.title 대체(동일 위치, 동일 문구)
...
def read_temperature_forecast(file):
    ...
    temp_col = next((c for c in df.columns if ("평균기온" in c) or ("기온"==c) or (c.lower() in ["temp","temperature"])), None)
    if temp_col is None:
...
if mode == "공급량 분석":
    with st.sidebar:
        ...
        if src == "Repo 내 파일 사용":
            ...
            # 기온예측 엑셀
            forecast_candidates = [data_dir / "기온예측.xlsx", *[Path(p) for p in glob(str(data_dir / "*기온예측*.xlsx"))]]
            if any(p.exists() for p in forecast_candidates):
                fc_path = next(p for p in forecast_candidates if p.exists())
                st.success(f"예상기온 파일 사용: {fc_path.name}")
                forecast_df = read_temperature_forecast(fc_path)
            else:
                st.warning("data 폴더에 '기온예측.xlsx'이 없습니다. 아래 업로드로 선택하세요.")
                up_fc = st.file_uploader("예상기온 파일 업로드(xlsx) — (날짜, 평균기온)", type=["xlsx"], key="up_fc_repo")
    ...
    if supply_df is None or len(supply_df)==0:
        st.info("실적 엑셀을 선택/업로드하세요.")
        st.stop()
    if forecast_df is None or forecast_df.empty:
        st.info("예상기온 엑셀을 선택/업로드하세요.")
        st.stop()
    ...
    if run_btn:
        ...
        # ① 예측기온(기온예측.xlsx) 매칭
        fc = forecast_df.copy()
        fut_base = fut_base.merge(fc, on=["연","월"], how="left")  # '예상기온'
        # ② 부족분은 학습데이터의 월평균으로 보강
        monthly_avg_temp = train_df.groupby("월")[temp_col].mean().rename("보강기온").to_frame().reset_index()
        ...
        if miss.any():
            ...
            st.warning("기온예측 파일에 없는 월은 학습데이터의 월평균기온으로 보강했어.")
    ...
    with c2:
        d_best = st.number_input("Best Δ°C", value=-0.5, step=0.5, format="%.1f", key="s_best")
    with c3:
        d_cons = st.number_input("Conservative Δ°C", value=+0.5, step=0.5, format="%.1f", key="s_cons")
    ...
    def add_scenario(df, name):
        ...
        return d
    tbl_all = pd.concat([
        add_scenario(tbl_n, "Normal"),
        add_scenario(tbl_b, "Best"),
        add_scenario(tbl_c, "Conservative")
    ], ignore_index=True)
    st.download_button(
    ...
    for prod in prods:
        ...
        ax.text(0.02, 0.96, f"Poly-3: {poly_eq_text(model)}", transform=ax.transAxes, ha="left", va="top", fontsize=9, color="#1f77b4",
                bbox=dict(boxstyle="round,pad=0.3", facecolor="white", alpha=0.75))
        plt.tight_layout(); st.pyplot(fig, clear_figure=True)

        # ② 기온-공급량 상관(Poly-3) + 95% 신뢰구간 + 온도별 중앙값
    ...
    # ▶ 95% 신뢰구간 설명(간단)
    st.caption("**95% 신뢰구간(근사 예측구간)**: 학습 데이터의 잔차 표준편차 *s*를 이용해 예측값 ± 1.96·s 로 표시. 정규·등분산 가정 하에서 새로운 관측의 약 95%가 이 범위에 들어간다.")

# =============== B) 판매량 분석(냉방용) =====================================
else:
    # 제목만 아이콘으로 교체(기존 위치/내용 동일)
    ...
    if run_btn:
        ...
        for c in ["기간평균기온","당월평균기온"]:
            ...
            if miss2.any():
                pred_base.loc[miss2,c] = pred_base.loc[miss2,"월"].map(fallback_by_M)
    ...
    if show_poly3:
        ...
        ax2.set_xlim(1,12); ax2.set.xticks(months); ax2.set_xticklabels([f"{mm}월" for mm in months])
        ...
        ax3.text(xmin + 0.02*(xmax-xmin), ymin + 0.06*(ymax-ymin), f"Poly-3: {poly_eq_text(model_s)}",
    ...
    if show_poly4:
        ...
        ax34.text(xmin4 + 0.02*(xmax-xmin), ymin4 + 0.06*(ymax-ymin), f"Poly-4: {poly_eq_text4(model_s4)}",
                  fontsize=10, bbox=dict(boxstyle="round,pad=0.3", facecolor="white", alpha=0.75))
        st.pyplot(fig34)
