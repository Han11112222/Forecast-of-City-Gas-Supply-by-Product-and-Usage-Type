...
if mode == "공급량 분석":
    with st.sidebar:
        ...
        if src == "Repo 내 파일 사용":
            ...
            # 기본 선택: 파일명에 '상품별공급량' 또는 '공급량'이 들어간 항목
            if repo_files:
...
# =============== B) 판매량 분석(냉방용) =====================================
    ...
    # 파일 선택 UI (본문에 표시하되, 불필요한 성공 배너는 제거)
    c1, c2 = st.columns(2)
    if sales_src == "Repo 내 파일 사용":
        ...
        if not repo_sales_path or not repo_temp_path:
            ...
            with c2: st.info(f"레포 파일: {repo_temp_path.name}")
