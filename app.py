...
def read_temperature_raw(file):
    ...
    for i in range(len(head)):
        ...
        if any(v in ["날짜", "일자", "date", "Date"] for v in row) and any(
            ("평균기온" in v)
            or ("기온" in v)
            or (isinstance(v, str) and v.lower() in ["temp", "temperature"])
            for v in row
        ):
            header_row = i
            break
...
if mode == "공급량 분석":
    ...
    with st.sidebar:
        ...
        if df is None or len(df) == 0:
            st.info("좌측에서 실적 엑셀을 선택/업로드하세요.")
            st.stop()

        title_with_icon("🧪", "학습 데이터 연도 선택", "h4")
        ...
        title_with_icon("⏱️", "예측 설정", "h4")
        last_year = int(df["연"].max())
        col1, col2 = st.columns(2)
        with col1:
            year_range = list(range(2010, 2036))
            start_y = st.selectbox("예측 시작(연)", year_range, index=year_range.index(last_year))
            end_y = st.selectbox("예측 종료(연)", year_range, index=year_range.index(last_year))
        with col2:
    ...
    # 시나리오
    title_with_icon("🌡️", "시나리오 Δ°C (평균기온 보정)", "h2")
    ...
    render_centered_table(
        tbl_n,
        float1_cols=["월평균기온"],
        int_cols=[c for c in tbl_n.columns if c not in ["연", "월", "월평균기온"]],
        index=False,
    )
    ...
    render_centered_table(
        tbl_b,
        float1_cols=["월평균기온"],
        int_cols=[c for c in tbl_b.columns if c not in ["연", "월", "월평균기온"]],
        index=False,
    )
    ...
    render_centered_table(
        tbl_c,
        float1_cols=["월평균기온"],
        int_cols=[c for c in tbl_c.columns if c not in ["연", "월", "월평균기온"]],
        index=False,
    )
    ...
    tbl_all = pd.concat(
        [add_scenario(tbl_n, "Normal"), add_scenario(tbl_b, "Best"), add_scenario(tbl_c, "Conservative")],
        ignore_index=True,
    )
    st.download_button(
    ...
    for prod in prods:
        ...
        P2 = P.copy()
        P2["연"] = P2["연"].astype(int)
        P2["월"] = P2["월"].astype(int)
        for _ in range(12):
        ...
        ax.set_xticklabels([f"{mm}월" for mm in months])
        ax.set_xlabel("월")
...
# =================== B) 판매량 분석(냉방용) ===================
    ...
    if run_btn:
        ...
        for m in months:
            ...
            avg_month = monthly_cal.loc[
                (monthly_cal["연"] == m.year) & (monthly_cal["월"] == m.month), "기온"
            ].mean()
    ...
    # ───────────── Poly-3 ─────────────
    if show_poly3:
        title_with_icon("🌡️", "시나리오 Δ°C (평균기온 보정) — Poly-3", "h2")
        c1, c2, c3 = st.columns(3)
        with c1:
        ...
        def forecast_sales_table(delta: float) -> pd.DataFrame:
            ...
            y_future, _, _, _ = fit_poly3_and_predict(
                x_train, y_train, base["기간평균기온(적용)"].values.astype(float)
            )
        ...
        render_centered_table(
            sale_n,
            float1_cols=["월평균기온(적용)", "기간평균기온(적용)"],
            int_cols=["예측판매량"],
            index=False,
        )
        ...
        # 검증
        title_with_icon("✅", "판매량 예측 검증 — Poly-3", "h3")
        ...
        render_centered_table(
            ...
            int_cols=["실제판매량", "예측판매량", "오차"],
            index=False,
        )

        # 그래프
        title_with_icon("📈", "그래프 (Normal 기준) — Poly-3", "h3")
        ...
        ax2.set_xticklabels([f"{mm}월" for mm in months])
        ax2.set_xlabel("월")
        ...
        ax2.text(
            0.02,
            0.96,
            f"Poly-3: {poly_eq_text(model_line)}",
        ...
        # 상관관계
        title_with_icon("🔬", f"기온-냉방용 실적 상관관계 (Train, R²={r2_fit:.3f}) — Poly-3", "h3")
    ...
    if show_poly4:
        ...
        title_with_icon("🌡️", "시나리오 Δ°C (평균기온 보정) — Poly-4", "h2")
        ...
        def forecast_sales_table_poly4(delta: float) -> pd.DataFrame:
            ...
            base["월평균기온(적용)"] = base["당월평균기온"] + delta
            base["기간평균기온(적용)"] = base["기간평균기온"] + delta
            y_future, _, _, _ = fit_poly4_and_predict(
                x_train, y_train, base["기간평균기온(적용)"].values.astype(float)
            )
            base["예측판매량"] = np.clip(np.rint(y_future).astype(np.int64), a_min=0, a_max=None)
            out = base[["연", "월", "월평균기온(적용)", "기간평균기온(적용)", "예측판매량"]].copy()
        ...
        title_with_icon("🙂", "Normal (Poly-4)", "h3")
        ...
        title_with_icon("🚀", "Best (Poly-4)", "h3")
        ...
        title_with_icon("🛡️", "Conservative (Poly-4)", "h3")
        ...
        # 검증
        title_with_icon("✅", "판매량 예측 검증 — Poly-4", "h3")
        ...
        # 그래프
        title_with_icon("📈", "그래프 (Normal 기준) — Poly-4", "h3")
        ...
        ax24.set_xticklabels([f"{mm}월" for mm in months])
        ax24.set_xlabel("월")
        ax24.set_ylabel("판매량 (MJ)")
        ax24.set_title(f"냉방용 — Poly-4 (Train R²={r2_line4:.3f})")
        ax24.legend(loc="best")
        ax24.grid(alpha=0.25)
        ax24.text(
            0.02,
            0.96,
            f"Poly-4: {poly_eq_text4(model_line4)}",
        ...
        # 상관관계
        title_with_icon("🔬", f"기온-냉방용 실적 상관관계 (Train, R²={r2_fit4:.3f}) — Poly-4", "h3")
        ...
        ax34.set_xlabel("기간평균기온 (℃)")
        ax34.set_ylabel("판매량 (MJ)")
        ax34.grid(alpha=0.25)
        ax34.legend(loc="best")
        xmin4, xmax4 = ax34.get_xlim()
        ymin4, ymax4 = ax34.get_ylim()
        ax34.text(
            xmin4 + 0.02 * (xmax4 - xmin4),
            ymin4 + 0.06 * (ymax4 - ymin4),
