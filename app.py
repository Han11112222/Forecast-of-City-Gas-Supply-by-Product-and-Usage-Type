...
if mode == "공급량 분석":
    ...
    for prod in prods:
        ...
        # ② 기온-공급량 상관(Poly-3) + 95% 신뢰구간 + (2) 온도별 중앙값(다른 색) ← 추가
        ...
        st.pyplot(figc)
        # ▶ 95% 신뢰구간 설명(간단)
        st.caption("**95% 신뢰구간(근사 예측구간)**: 학습 데이터의 잔차 표준편차 *s*를 이용해 "
                   "`예측값 ± 1.96·s` 로 표시. 정규·등분산 가정 하에서 새로운 관측의 약 95%가 이 범위에 들어간다.")
...
# =============== B) 판매량 분석(냉방용) =====================================
    ...
    if show_poly3:
        ...
        # 그래프 2: 기온-판매량 산점 + Poly3 + 95% 신뢰구간 + 방정식 (기존 그대로)
    ...
    if show_poly4:
        ...
        # 그래프 2: 기온-판매량 산점 + Poly-4 + 95% 신뢰구간 (기존 유지)
